...
def synthesize_consolidated_summary(summary_list):
    """
    Takes a list of summaries generated by different AI models, 
    merges their unique key points, removes overlapping/duplicate details, 
    and returns a unified master summary.
    """
    if not summary_list:
    ...
    prompt = (
        "You are an executive summary synthesizer. Below are multiple summaries of the same meeting generated by different AI models:\n\n"
        f"{combined_raw_text}\n\n"
        "Tasks:\n"
        "1. Identify the key decisions, main discussions, and action items across all summaries.\n"
        "2. Synthesize them into a single, highly cohesive Master Summary.\n"
        "3. Remove repetitive points and redundant statements across models.\n"
        "4. Format the final output clearly using clean bullet points (•)."
    )
    
    # Use Groq or OpenAI to perform synthesis
    if groq_client:
        try:
            res = groq_client.chat.completions.create(
                model="llama-3.3-70b-versatile",
    ...
    try:
        ...
        if 'text' in transcript_dict and transcript_dict['text']:
            ...
            if consolidated_summary_list:
               ...
               consolidated_summary = synthesize_consolidated_summary(consolidated_summary)
